...
python3 src/data/load_data.py --config "$CONFIG"

echo "[2/12] Preprocessing..."
python3 src/data/preprocess_pm10.py --config "$CONFIG"

echo "[3/12] Creating LightGBM features..."
python3 src/data/make_features_lgbm.py --config "$CONFIG"

echo "[4/12] Creating LSTM sequences..."
python3 src/data/make_sequences_lstm.py --config "$CONFIG"

echo "[5/12] Creating splits (rolling-origin + holdout)..."
python3 src/data/make_splits.py --config "$CONFIG"

# Phase 2: Training
echo "[6/12] Training persistence baseline..."
python3 src/training/train_persistence.py --config "$CONFIG"

echo "[7/12] Training LightGBM..."
python3 src/training/train_lgbm.py --config "$CONFIG"

echo "[8/12] Training LSTM..."
python3 src/training/train_lstm.py --config "$CONFIG"

# Phase 3: Metrics computation
echo "[9/12] Computing metrics by horizon..."
python3 src/evaluation/compute_metrics_by_horizon.py --config "$CONFIG"

echo "[10/12] Computing event metrics..."
python3 src/evaluation/compute_event_metrics.py --config "$CONFIG"

# Phase 4: Analysis
echo "[11/12] Comparing protocols..."
python3 src/evaluation/compare_protocols.py --config "$CONFIG"

echo "[Building canonical table..."
python3 src/evaluation/build_canonical_table.py --config "$CONFIG"

# Phase 5: Visualization
echo "[12/12] Plotting master figure..."
python3 src/plotting/plot_master_figure.py --config "$CONFIG"
