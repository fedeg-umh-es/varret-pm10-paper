...
still preserve little information about concentration amplitude or episode
evolution if its predictions collapse toward a near-constant value.
...
RMSE-relative skill should not be read as evidence of preserved amplitude or
event-detection fidelity without complementary diagnostics.
\end{abstract}
...
more accurate than persistence at medium leads while the corresponding
prediction sequences become under-variable and less informative about episode
evolution.
...
\begin{itemize}
  ...
  \item We introduce Skill$_{VP}$ as a simple diagnostic auxiliary metric that
    ...
    further check on event-detection fidelity in this case study.
\end{itemize}
...
retention when discussing horizon-wise behavior, without claiming that such
...
\end{equation}
When the origin observation is provider-invalid or absent, $y_t$ in this
definition is operationalized as the most recent causally available validated
observation, consistently with the forward-fill input policy; no future value
is used.
...
$(p,d,q)(P,D,Q)_s = (1,0,1)(1,0,0)_{24}$ and recursive multi-step forecasting
\citep{boxjenkins2015}.
This specification is fixed in the canonical producer for the diagnostic
rerun.  The repository does not preserve inspectable immutable evidence of its
selection before the rerun, so it is not presented as a tuned or selected
winner.
...
As a supportive event-detection complement, we evaluate exceedance detection at
...
event-oriented evaluation shows the corresponding loss of peak-detection
fidelity at long leads in this case study.
...
itself demonstrate predictive discrimination or decision-relevant usefulness.
In this study, estimating each threshold only from the corresponding fold's
training window avoids fitting the threshold to verification outcomes; it does
not guarantee stable event base rates.  Recall is therefore reported jointly
with precision and flag rate.
...
The canonical empirical package is identified by commit
\texttt{f57f076078760af8a88bd87815fdf94ab0064fa3}; its authoritative producer is
\texttt{scripts/run\_paper\_a\_empirical.py}, and the checked-in provenance
manifest is \texttt{docs/p4\_canonical\_provenance\_manifest.json}.
...
URL and SHA-256 checksum, row-level rolling-origin and holdout predictions under
\texttt{outputs/reproduction/}, and exact empirical dependencies in
\texttt{requirements-reproduction.txt}.  The repository also includes scripts
that recover the station series and reproduce predictions, metrics, event
