...
rewriting. All numerical elements remain downstream of the canonical
...
| T3 | Table | What retained counts accompany the sensitivity percentages? | verified `rule_b_sensitivity.csv` | `generate_rule_sensitivity.py` | 3 x 3 grid | Retained cells and change percentages | Preserves exact sensitivity counts beside the heatmap | Detailed table | MOVE_TO_SUPPLEMENT |
| T4 | Table | What was evaluated? | canonical provenance and verified labels | manual LaTeX table | five families | Model inputs and recoverability limits | Reproducibility support | Existing specification table | MOVE_TO_SUPPLEMENT |
...
argument and are not included in the v5 archive.
