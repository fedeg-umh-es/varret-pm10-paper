...
def test_alpha_near_one_when_predicted_variability_matches_observed() -> None:
    ...
    assert abs(summary.loc[0, "alpha"] - 1.0) < 1e-9


def test_alpha_near_zero_when_predictions_are_flat() -> None:
    predictions = pd.DataFrame(
        {
            "dataset": ["pm10_daily"] * 4,
            "model": ["test_model"] * 4,
    ...
    summary = build_variance_retention_summary(predictions, _build_skill_df(0.2))
    assert summary.loc[0, "alpha"] == 0.0
...
def test_near_ideal_flag_when_skill_positive_and_alpha_near_one() -> None:
    predictions = pd.DataFrame(
        {
            "dataset": ["pm10_daily"] * 4,
            "model": ["test_model"] * 4,
